...
"""Funciones para obtener la data de los periodos 2006, 2005, 2004 y 20003"""

from bs4 import BeautifulSoup
from utilitarios import report_html, find_tag

def report_data_2006(rpj, trimestre, anho):
    """Utiliza data de los periodos 2006"""
    ...
    data.update(balance_general_2006(url_balance_general))
    ...
    #En caso de que no existe, devuelve false
    data.update(ganancias_perdidas_2006(url_ganancias_perdidas))
    ...
    #En caso de que no existe, devuelve false
    #data.update(flujo_efectivo_2006(url_flujos_efectivo))
    ...
    #Agrega el diccionario resultado de flujos de efectivo a data
    #En caso de que no existe, devuelve false
    data.update(estado_cambios_patrimonio_2006(url_cambios_patrimonio))

    return data


#Funcion que devuelve datos del Estado de flujos de efectivo formato 2006
def balance_general_2006(url_balance_general):
    ...
    if html:
        ...
        data['cuentas_cobrar'] = \
        float(find_tag(reporte, u'Cuentas por Cobrar Comerciales ' + \
        u'(neto de provisión acumulada)/', 9).replace(',', ''))

        #Inversiones de capital
        data['inv_capital'] = \
        float(find_tag(reporte, u'Total Activo No Corriente/', 6). \
                                                  replace(',', ''))

        #Activo circulante
        data['act_circulante'] = \
        float(find_tag(reporte, u'Total Activo Corriente/', 6). \
                                               replace(',', ''))

        #Activos totales
        data['activos'] = \
        data['inv_capital'] + data['act_circulante']

        #Deuda corto plazo
        data['deuda_corto_plazo'] = \
        float(find_tag(reporte, u'Total Pasivo Corriente/', 6). \
                                                replace(',', ''))

        #Deuda largo plazo
        data['deuda_largo_plazo'] = \
        float(find_tag(reporte, u'Total Pasivo No Corriente/', 6). \
                                                   replace(',', ''))

        data['valor_libros'] = \
        float(find_tag(reporte, u'Total Patrimonio Neto/', 6). \
                                                   replace(',', ''))
...
def ganancias_perdidas_2006(url_ganancias_perdidas):
...
def flujo_efectivo_2006(url_flujos_efectivo):
...
def estado_cambios_patrimonio_2006(url_cambios_patrimonio):
