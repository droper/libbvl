...
def report_data_2002(rpj, trimestre, anho):
    """Utiliza data de los periodos 2002 y 2001"""
...
#Funcion que devuelve datos del Estado de flujos de efectivo formato 2002
def balance_general_2002(url_balance_general):
    ...
    if html:
            ...
            data['cuentas_cobrar'] = \
            float(find_tag(reporte, u'Cuentas por Cobrar a Largo Plazo',9).replace(',',''))
            + float(find_tag(reporte, u'Cuentas por Cobrar a Vinculadas a Largo Plazo',9).replace(',',''))
            + float(find_tag(reporte, u'Otras Cuentas por Cobrar a Largo Plazo',9).replace(',',''))
...
#Funcion que devuelve datos del Estado de Ganancias y perdidas formato 2002
