...
"""Funciones para obtener los datos de los periodos 2010, 2009, 2008 y 2007"""
...
def flujo_efectivo_2010(url_flujos_efectivo):
    ...
    if html:
        ...
        + u'Equivalente de Efectivo Provenientes de Actividades de ' + \
        u'Operación', 6).replace(',', ''))
...
def balance_general_2010(url_balance_general):
    ...
    if html:
        ...
        #Activos
        data['activos'] = float(find_tag(reporte, u'TOTAL ACTIVO/', 6). \
                                                        replace(',',''))

        #Cuentas por cobrar
        data['cuentas_cobrar'] = \
        float(find_tag(reporte, u'Cuentas por Cobrar Comerciales (neto)/', 9) \
                                                            .replace(',',''))
